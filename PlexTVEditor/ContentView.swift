...
struct SettingsView_New: View {
    ...
    struct SectionTrashConfirmation: Identifiable {
        let id = UUID()
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                        ...
                        if !viewModel.plexLibrarySections.isEmpty {
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                ...
                                HStack(spacing: 12) {
                                    ...
                                    if hasQueuedSectionJob {
                                        Text("Queued: \(viewModel.lastQueuedPlexSectionAction) in \(viewModel.lastQueuedPlexSectionLabel)")
                                            .font(.system(size: 11))
                                            .foregroundColor(.plexTextSecondary)
                                            .lineLimit(1)
                                    }
                                ...
                                if viewModel.isEmptyingPlexSection {
                                    HStack(spacing: 8) {
                                        ProgressView()
                                            .progressViewStyle(CircularProgressViewStyle(tint: .plexOrange))
                                        Text("Queueing Plex empty trash...")
                                            .font(.system(size: 12))
                                            .foregroundColor(.plexTextSecondary)
                                    }
                                }

                                if viewModel.isCancellingPlexSectionJob {
                                    HStack(spacing: 8) {
                                        ProgressView()
                                            .progressViewStyle(CircularProgressViewStyle(tint: .plexOrange))
                                ...
                                if viewModel.isRunningBulkPlexSectionMaintenance {
                                    HStack(spacing: 8) {
                                        ProgressView()
                                            .progressViewStyle(CircularProgressViewStyle(tint: .plexOrange))
                                        Text(viewModel.bulkPlexSectionMaintenanceProgress.isEmpty
                                             ? "Queueing bulk section maintenance..."
                                             : "Bulk maintenance: \(viewModel.bulkPlexSectionMaintenanceProgress)")
                                            .font(.system(size: 12))
                                            .foregroundColor(.plexTextSecondary)
                                    }
                                }
                ...
                SectionCard(title: "Section Job History", icon: "clock.badge.checkmark") {
                    VStack(alignment: .leading, spacing: 12) {
                        HStack(spacing: 12) {
                            ActionButton(
                        ...
                        if tvSectionHistory.isEmpty && movieSectionHistory.isEmpty {
                            Text("No section history for the currently selected TV/Movie sections yet.")
                                .font(.system(size: 12))
                                .foregroundColor(.plexTextSecondary)
                        } else {
                            VStack(alignment: .leading, spacing: 10) {
                                if !tvSectionHistory.isEmpty {
                                    Text("TV Section History")
                                        .font(.system(size: 12, weight: .semibold))
                                        .foregroundColor(.plexTextPrimary)

                                    ForEach(Array(tvSectionHistory.prefix(5)), id: \.id) { entry in
                                        HStack(spacing: 10) {
                                            Text(viewModel.formattedSectionActionDate(entry.timestamp))
                                                .font(.system(size: 11))
                                                .foregroundColor(.plexTextSecondary)
                                ...
                                if !movieSectionHistory.isEmpty {
                                    Text("Movie Section History")
                                        .font(.system(size: 12, weight: .semibold))
                                        .foregroundColor(.plexTextPrimary)

                                    ForEach(Array(movieSectionHistory.prefix(5)), id: \.id) { entry in
                                        HStack(spacing: 10) {
                                            Text(viewModel.formattedSectionActionDate(entry.timestamp))
                                                .font(.system(size: 11))
                                                .foregroundColor(.plexTextSecondary)
        ...
        .alert(item: $pendingSectionTrashConfirmation) { pending in
            Alert(
                title: Text("Empty Plex Trash?"),
                message: Text("This will remove unavailable items from \(pending.sectionLabel). This cannot be undone."),
                primaryButton: .destructive(Text("Empty Trash")) {
                    viewModel.emptyTrashForSelectedPlexSection(
